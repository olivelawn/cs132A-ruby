...
#the_string.squeeze(" ").gsub(/^\s/, '').each { |line| puts "#{line.capitalize}" }
the_string.squeeze(" ").scan(/^(\W*)(.*)$/) {|x,y| puts x + y.capitalize }
...
#the_string.squeeze(" ").gsub(/^\s/, '').each { |line| line.capitalize.sub(/[xX]$/, '').each_byte { |c| puts c } }
#puts
#puts
...
puts "C|Dec|Hex"
puts "---------"
the_string.squeeze(" ").gsub(/^\s/, '').each { |line| line.capitalize.sub(/[xX]$/, '').each_byte { |c| printf("%c|%d|0x%x\n", c, c, c) } }
puts
puts
...
the_string.replace(the_string[0,100].strip.reverse.squeeze.upcase)
puts "#{the_string}"
puts "the_string object_id is #{the_string.object_id}"
puts
